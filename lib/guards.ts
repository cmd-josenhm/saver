...
export function providerDownloadHosts(): Set<string> {
  const configured = (process.env.MEDIA_DOWNLOAD_HOSTS ?? "")
    .split(",")
  ...
  try {
    const provider = new URL(process.env.MEDIA_PROVIDER_URL ?? "");
    return new Set([provider.hostname.toLowerCase().replace(/^www\./, "")]);
  } catch {
    return new Set();
  }
}
