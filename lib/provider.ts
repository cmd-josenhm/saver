...
export async function createJob(url: string, source: string): Promise<Job> {
  ...
  if (!response.ok) {
    ...
    if (code === "PRIVATE_PROFILE") throw new ProviderError("PRIVATE_PROFILE", 403);
...
export async function getJob(id: string): Promise<Job> {
  ...
  if (!response.ok) {
    throw new ProviderError(response.status === 404 ? "JOB_NOT_FOUND" : "PROVIDER_ERROR", response.status === 404 ? 404 : 502);
  }
