...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!/^[A-Za-z0-9_-]{8,128}$/.test(id)) return NextResponse.json({ error: "Job invalide." }, { status: 400 });
  if (!verifyJobToken(id, req.headers.get("x-job-token"))) return NextResponse.json({ error: "Accès au job refusé." }, { status: 401 });

  try {
    if (!(await enforceRateLimit("poll:" + ip(req) + ":" + id, 60, "1 m"))) {
    ...
    if (error instanceof ProviderError && error.code === "JOB_NOT_FOUND") return NextResponse.json({ error: "Job introuvable." }, { status: 404 });
    if (error instanceof Error && error.message === "RATE_LIMIT_NOT_CONFIGURED") return NextResponse.json({ error: "Configuration de production incomplète." }, { status: 503 });
