...
export async function GET() {
  ...
  const downloadHostsConfigured = Boolean(process.env.MEDIA_DOWNLOAD_HOSTS);
  ...
  const ok = providerConfigured && downloadHostsConfigured && rateLimitConfigured && jobSecretConfigured;

  return NextResponse.json(
    { ok, service: "saver", providerConfigured, downloadHostsConfigured, rateLimitConfigured, jobSecretConfigured },
